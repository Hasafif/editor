...
export const advanced_check = async function run(text) {
    try {
...
export const autoncomplete = async (userText: string) => {
	const text = stripHtmlTags(userText);
	console.log(userText)
	if (text !== "") {
		const { data } = await axios.post("/complete", {
			textTocomplete: text,
		});
		const { aiPrompt } = data;
		return aiPrompt;
...
export const pdf = async function (text) {
